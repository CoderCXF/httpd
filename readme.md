...
|       Part Ⅰ        |      Part Ⅱ      |        Part Ⅲ        |       Part Ⅳ       |          Part Ⅴ           |       Part Ⅵ        |   Part VII    |     Part Ⅲ    |
| :-----------------: | :--------------: | :------------------: | :----------------: | :-----------------------: | :-----------------: | :-----------: | :------------------: |
| [Inroduce](#1.介绍) | [Build](#2.编译) | [Point](#3.技术要点) | [Timer](#4.Timer) | [Log](#5.异步日志) | [Buffer](#6.应用层buffer) | [Test](#7.压力测试) | [FAQ](#8.FAQ) |
...
这里使用的是**红黑树**，在C++中就是 `map` 和 `set` 这两个集合。

键就是定时器的过期时间，而值就是对应的该定时器对象。因为同样的过期时间的定时器可能所做的工作不一样（也就是回调函数不一样），而 `map` 是只保存不重复的 `key`，所以不能简单的使用 `map`，转而可以使用`multi_map`，但是`multi_map` 集合不熟悉，所以还是使用 `map` 和 `set` 结合进行使用。

在本项目中使用的是如下结构保存所有的 `Timer`:
...
首先定时器是由三个类组成的：`Timer` 、 `TimerQueue` 以及`TimerId`。

* `Timer` 是对定时器的封装，其主要的属性如下所示：

  ```c++
  const TimerCallback callback_; // 定时器回调函数
  Timestamp expiration_;         // 下一次的超时时刻
  const double interval_;        // 超时时间间隔（重复触发），如果是一次性定时器的话，该值为0
  const bool repeat_;            // 是否重复，如果是一次性定时器的话，
  const int64_t sequence_;       // 定时器序列号，每一个定时器都有一个序列号
  
  static AtomicInt64 s_numCreated_;  // 定时器计数，当前已经创建的定时器数量是一个原子性操作
  ```

* `TimerId` 是一个对Timer的高级抽象。主要成员就是定时器对象 `Timer`。

* `TimerQueue` 的接口很简单，只有两个 `addTimer` 和 `cancel`。主要是用来添加定时器至队列（map集合中）以及取消定时器的操作。

实现逻辑图：

<img src="https://gitee.com/codercxf/Blog_image_hexo/raw/master/2021/20210602192248.png" style="zoom: 80%;" />

* 在 `TimerQueue` 的构造函数中，注册该定时器所在的 `Channel` 的可读事件以及回调函数。

* 具体的操作是在`TimeQueue` 的构造函数中调用`Channel::setReadCallback(TimeQueue::headleRead)`  ，然后调用 `Channel::enableReading()` 注册 `EPOLL_READ` 事件；

* 调用 `Channel::update()` 函数，然后调用 `EventLoop` 的 `update` 函数，然后调用poller 的`updateCahnnel` 函数，` update(EPOLL_CTL_ADD, *channel*)`，将该通道的可读事件挂在树上监听。

  

<img src="https://gitee.com/codercxf/Blog_image_hexo/raw/master/2021/20210602193025.png" style="zoom: 67%;" />

1. 定时器超时，产生可读事件；

2. `epoll_wait` 函数返回活跃的通道，然后调用 `Channel` 的 `headleEvent` 函数；

3. `Channel::handleEvent` 调用该通道注册的可读事件的回调函数 `TimerQueue::handleRead()`;

4. 在`TimerQueue::handleRead` 函数中获取所有的已经超时的定时器列表（`TimerQueue::getExpired` 函数实现），并且依次回调超时定时器列表中的每一个定时器的回调函数，回调函数是由用户注册的。
